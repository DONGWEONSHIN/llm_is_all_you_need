...
$(document).ready(function() {                      // .ready document가 실행되면 한 번만 로드되는 함수
  $("#messageArea").on("submit", function(event) {  // .on id가 messageArea의 form 태그에 submit 이벤트가 발생하면 function() 함수를 실행
      ...
      $.ajax({                                      // ajax로 서버와 통신
          ...
          type: "POST",
          url: "/chatWithPdf",                             //  /chat /chatWithPdf
    ...
    //
    $("#messageAreaMuseum").on("submit", function(event) {  // .on id가 messageArea의 form 태그에 submit 이벤트가 발생하면 function() 함수를 실행
      const date = new Date();                      // 날짜
      const hour = date.getHours();                 // 시간
      const minute = date.getMinutes();             // 분
      const str_time = hour+":"+minute;             // 시간 : 분
    ...
    //
    $("#messageAreaDiffusion").on("submit", function(event) {  // .on id가 messageArea의 form 태그에 submit 이벤트가 발생하면 function() 함수를 실행
      const date = new Date();                      // 날짜
      const hour = date.getHours();                 // 시간
      const minute = date.getMinutes();             // 분
      const str_time = hour+":"+minute;             // 시간 : 분
    ...
    //
    $("#uploadForm").on("submit", function(event) { // uploadForm에 submit이 발생하면 function() 함수를 실행
